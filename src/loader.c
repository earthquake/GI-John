	...
	struct db_options *db_options, char *line)
{
	...
	*login = ldr_get_field(&line, db_options->field_sep_char);
	...
	if (!*(*ciphertext = ldr_get_field(&line, db_options->field_sep_char)) && !line) {
		if (strlen(*login) < 13)
			return 0;
		*ciphertext = *login;
		*login = no_username;
 ...
 */
	if (!gijohnpresent) pw->binary = NULL;
}
 ...
 */
		if (*format == &fmt_crypt &&
			...
			continue;
#endif
		if ((count = (*format)->methods.valid(*ciphertext)) && !(strncmp((*format)->params.label, xmlformat,
                (strlen((*format)->params.label)>strlen(xmlformat))?strlen((*format)->params.label):strlen(xmlformat)))) {
			fmt_init(*format);
			if (db_options->single_salts)
			{
				char *salt = (char*)(*format)->methods.salt(*ciphertext);
